    ...
    def _copy(self, file_list, src, dest):
        ...
        for f in file_list:
            ...
            if os.path.isdir(srcpath):
                ...
                log.debug('Copied directory \"' + os.path.basename(srcpath) + '\" from \"' + os.path.dirname(srcpath) + '\" to \"' + dest + '\"')
            else:
                shutil.copy2(srcpath, dest)
                self.file_copied_count = self.file_copied_count + 1
                log.debug('Copied \"' + os.path.basename(srcpath) + '\" from \"' + os.path.dirname(srcpath) + '\" to \"' + dest + '\"')
...
def dispatchChecker():
    ...
    while 1:
        ...
        log.debug('Total files copied ' + str(my_dispatch.file_copied_count))
        log.debug('Total folders copied ' + str(my_dispatch.folder_copied_count))
